...
|bits-priority
|Bits reserved for Interrupt Priority. +
Undeclared means Interrupt Priority is unsupported.
|<u32> +
Ignored if `max-priority` is undeclared or is set to zero.
|PLIC design specific
...
|int-priority-block
|Offset to Interrupt Source Priority. +
Undeclared means Interrupt Priority is unsupported.
|<u32> +
Ignored if `max-priority` is undeclared or is set to zero.
|PLIC design specific
...
|int-pending-block
|Offset to Interrupt Pending block.
|<u32>
|PLIC design specific

|int-enable-block
|Offset to Interrupt enable block.
|<u32>
|PLIC design specific
...
|<prop-encoded-array> +
This property describes the interrupt operation parameters of configuring `nint` interrupt sources on specific RISC-V Hart context. Multiple `int-context` properties could be declared in PLIC Device Tree. If multiple `int-context` are declared, the secquence of `int-context` must be in the same order of supported contexts which organized in Interuupt Enable block. Platform vendor shall spec-out the PLIC design in platform specification. +
 +
...
The privilege mode of this context. The value could be one of below, +
  ...
  "U-Mode" +
 +
*nint-conetxt:* _<u32>_ +
Number of interrupt sources occupied to this context. `nint-conetxt` may smaller then `nint`. +
 +
*interrupt-threshold-reg:* _<u32>_ +
The offset to Interrupt Threshold register within PLIC memory map. Ignored if `max-priority` is undeclared or is set to zero. +
 +
*interrupt-claim-reg:* _<u32>_ +
The offset to Interrupt Claim register of this context within PLIC memory map. +
 +
*interrupt-complete-reg:* _<u32>_ +
The offset to Interrupt Complete register of this context within PLIC memory map. +
 +
|PLIC design specific

|===
			
### For the platform the OS image with device driver only supports the specific RISC-V PLIC on platform:

In this use case, the Device Tree of RISC-V PLIC could be small and simple. Below Device Tree takes SiFive PLIC as reference,

        interrupt-controller@c000000 {
　　　　　　　　　#interrupt-cells = <0x00000001>;
　　　　　　　　　compatible = "riscv,plic", "sifive, plic";
　　　　　　　　　interrupt-controller;
...
### For the single OS image with the device driver supports RISC-V PLICs which compliant with this specification:
...
　　　　　　　　　reg = <0x00000000 0x0c000000 0x00000000 0x04000000>;
　　　　　　　　　reg-names = "control";
　　　　　　　　　riscv,nint = <0x00000400>;
　　　　　　　　　riscv,ndev = <0x00000035>;			
　　　　　　　　　riscv,max-priority = <0x00000007>;
　　　　　　　　　riscv,bits-priority = <0x00000020>;
　　　　　　　　　riscv,int-priority-block　= <0x00000000>;	
　　　　　　　　　riscv,int-pending-block = <0x00001000>;
　　　　　　　　　riscv,int-enable-block = <0x00002000>;
　　　　　　　　　riscv,int-context = <&cpu0 0x00000000 "M-Mode" 0x00000040 0x00200000 0x00200004 0x00200004>;
　　　　　　　　　riscv,int-context = <&cpu0 0x00000001 "M-Mode" 0x00000040 0x00201000 0x00201004 0x00201004>; 
　　　　　　　　　riscv,int-context = <&cpu0 0x00000001 "S-Mode" 0x00000040 0x00202000 0x00202004 0x00202004>;
　　　　　　　　　riscv,int-context = <&cpu0 0x00000002 "M-Mode" 0x00000040 0x00203000 0x00203004 0x00203004>; 
　　　　　　　　　riscv,int-context = <&cpu0 0x00000002 "S-Mode" 0x00000040 0x00204000 0x00204004 0x00204004>; 
　　　　　　　　　riscv,int-context = <&cpu0 0x00000003 "M-Mode" 0x00000040 0x00205000 0x00205004 0x00205004>;
　　　　　　　　　riscv,int-context = <&cpu0 0x00000003 "S-Mode" 0x00000040 0x00206000 0x00206004 0x00206004>; 
　　　　　　　　　riscv,int-context = <&cpu0 0x00000004 "M-Mode" 0x00000040 0x00207000 0x00207004 0x00207004>; 
　　　　　　　　　riscv,int-context = <&cpu0 0x00000004 "S-Mode" 0x00000040 0x00208000 0x00208004 0x00208004>; 			
        };		
